...
export const FIVERR = "https://www.fiverr.com/shopifybash";
...
export const FAQ_ITEMS = [
  ...
  { q: "Will I work with you directly?", a: "Yes. You always work with me, no junior account managers and no agency middle layer." },
...
export function About({ heading = true }: { heading?: boolean }) {
  return (
    <section id="about" className={heading ? "py-20 sm:py-28" : "pb-20 pt-8"}>
      <div className="mx-auto max-w-5xl px-4 sm:px-6">
        {heading && <SectionHeading eyebrow="About" title={<>The Specialist Behind <span className="gradient-text">The Results</span></>} />}
...
function ServiceRow({ items }: { items: { i: any; t: string; d: string }[] }) {
  ...
  return (
    <div className="relative group/row">
      <div ref={ref} className="flex gap-4 overflow-x-auto snap-x snap-mandatory pb-4 -mx-4 px-4 scroll-smooth" style={{ scrollbarWidth: "thin" }}>
      ...
      <button onClick={() => scroll(-1)} aria-label="Previous services" className="hidden md:grid absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 h-10 w-10 place-items-center rounded-full glass hover:bg-foreground/10 opacity-0 group-hover/row:opacity-100 transition">
        <ChevronLeft className="h-4 w-4" />
      </button>
      <button onClick={() => scroll(1)} aria-label="Next services" className="hidden md:grid absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 h-10 w-10 place-items-center rounded-full glass hover:bg-foreground/10 opacity-0 group-hover/row:opacity-100 transition">
...
export function Services({ heading = true, showAllLink = true }: { heading?: boolean; showAllLink?: boolean }) {
  const half = Math.ceil(SERVICE_ITEMS.length / 2);
  return (
    <section id="services" className={heading ? "py-20 sm:py-28" : "pb-20 pt-8"}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        {heading && <SectionHeading eyebrow="Services" title={<>Full-Service Shopify <span className="gradient-text">Optimization</span></>} />}
        <div className="mt-12 space-y-5 reveal">
          <ServiceRow items={SERVICE_ITEMS.slice(0, half)} />
          <ServiceRow items={SERVICE_ITEMS.slice(half)} />
        </div>
        {showAllLink && (
          <div className="mt-8 flex justify-center">
            <Link to="/services" className="inline-flex items-center gap-2 rounded-xl glass hover:bg-foreground/10 px-6 py-3 text-sm font-semibold transition">
...
export function ServicesGrid() {
  return (
    <section className="pb-20 pt-6">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-5 reveal">
        {SERVICE_ITEMS.map(({ i: Icon, t, d }) => (
...
export function Process({ heading = true }: { heading?: boolean }) {
  ...
  return (
    <section id="process" className={heading ? "py-20 sm:py-28" : "pb-20 pt-8"}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          {heading && <SectionHeading eyebrow="Process" title={<>From Audit to Launch, <span className="gradient-text">A Proven System</span></>} align="left" />}
          <div className="flex gap-2 ml-auto">
            <button onClick={() => scroll(-1)} aria-label="Previous step" className="h-10 w-10 grid place-items-center rounded-xl glass hover:bg-foreground/10">
              <ChevronLeft className="h-4 w-4" />
            </button>
            <button onClick={() => scroll(1)} aria-label="Next step" className="h-10 w-10 grid place-items-center rounded-xl glass hover:bg-foreground/10">
              <ChevronRight className="h-4 w-4" />
            </button>
          </div>
        </div>
        <div className="mt-12 relative reveal">
          <div ref={ref} className="overflow-x-auto pb-6 -mx-4 px-4 scroll-smooth" style={{ scrollbarWidth: "thin" }}>
...
function ProjectCard({ p }: { p: Project }) {
  ...
  return (
    <article className="glass rounded-2xl overflow-hidden shadow-card hover:shadow-glow transition group h-full flex flex-col">
      <a href={p.url} target="_blank" rel="noopener noreferrer" className="block aspect-[16/10] overflow-hidden bg-royal/10 relative">
        {failed ? (
          <div className="absolute inset-0 grid place-items-center gradient-primary/10">
            <span className="font-display text-2xl font-bold gradient-text">{p.name}</span>
          </div>
        ) : (
          <img
            src={shot(p.url)}
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <a href={p.url} target="_blank" rel="noopener noreferrer" className="mt-auto pt-4 inline-flex items-center gap-1.5 text-sm font-medium text-cyan hover:gap-2 transition-all">
...
export function Portfolio({ heading = true }: { heading?: boolean }) {
  ...
  return (
    <section id="portfolio" className={heading ? "py-20 sm:py-28" : "pb-20 pt-8"}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          {heading && <SectionHeading eyebrow="Portfolio" title={<>Stores Built &amp; Optimized to This Standard</>} align="left" />}
          <div className="flex gap-2 ml-auto">
            <button onClick={() => scroll(-1)} aria-label="Previous project" className="h-10 w-10 grid place-items-center rounded-xl glass hover:bg-foreground/10">
              <ChevronLeft className="h-4 w-4" />
            </button>
            <button onClick={() => scroll(1)} aria-label="Next project" className="h-10 w-10 grid place-items-center rounded-xl glass hover:bg-foreground/10">
              <ChevronRight className="h-4 w-4" />
            </button>
          </div>
        </div>
        <div ref={ref} className="mt-10 flex gap-5 overflow-x-auto snap-x snap-mandatory pb-6 -mx-4 px-4 scroll-smooth" style={{ scrollbarWidth: "thin" }}>
          {PROJECTS.map((p) => (
            <div key={p.url} className="snap-start shrink-0 w-[300px] sm:w-[380px] lg:w-[440px]">
              <ProjectCard p={p} />
            </div>
          ))}
        </div>
        {heading && (
          <div className="mt-6 flex justify-center">
            <Link to="/portfolio" className="inline-flex items-center gap-2 rounded-xl glass hover:bg-foreground/10 px-6 py-3 text-sm font-semibold transition">
...
export function PortfolioGrid() {
  return (
    <section className="pb-20 pt-6">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-6 reveal">
...
export function CaseStudies({ heading = true }: { heading?: boolean }) {
    ...
    >
      ...
      <div className={`p-6 ${big ? "lg:p-8" : ""}`}>
        ...
        <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-cyan/30 bg-royal/10 px-3 py-1.5 text-xs font-semibold text-cyan">
          <TrendingUp className="h-3.5 w-3.5" /> {s.impact}
        </div>
      </div>
    </button>
  );
  ...
  return (
    <section id="case-studies" className={heading ? "py-20 sm:py-28" : "pb-20 pt-8"}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        {heading && (
          <SectionHeading
            eyebrow="Case Studies"
            title={<>Real Store Optimization <span className="gradient-text">Case Studies &amp; Results</span></>}
            subtitle="Watch walkthroughs of actual Shopify and dropshipping store fixes SEO recovery, tracking repairs, and performance gains."
          />
        )}
        <div className="mt-12">
          <BuyerReviewFeature />
        ...
        <div className="mt-6 grid lg:grid-cols-2 gap-6 reveal">
          <Card s={HOME_CASE_STUDIES[0]} index={0} big />
          <div className="grid gap-6">
            {HOME_CASE_STUDIES.slice(1).map((s, i) => <Card key={s.title} s={s} index={i + 1} />)}
          </div>
        </div>
      ...
      {current && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-background/90 backdrop-blur-md p-4 animate-fade-up" onClick={() => setActive(null)} role="dialog" aria-modal="true">
          ...
          <div className="w-full max-w-5xl aspect-video rounded-2xl overflow-hidden shadow-elegant border border-border bg-black" onClick={(e) => e.stopPropagation()}>
            <video key={current.src} src={current.src} poster={current.poster} controls autoPlay playsInline preload="metadata" className="w-full h-full" />
          </div>
        </div>
      )}
    </section>
...
/* ---------- DIAGNOSIS ---------- */
export function Diagnosis({ heading = true }: { heading?: boolean }) {
  ...
  return (
    <section id="diagnosis" className={heading ? "py-20 sm:py-28" : "pb-20 pt-8"}>
      <div className="mx-auto max-w-4xl px-4 sm:px-6">
        {heading && (
          <SectionHeading
            eyebrow="Free Diagnosis"
            title={<>Get a <span className="gradient-text">Free Shopify Store Diagnosis</span></>}
            subtitle="I will analyze your store and identify what is stopping your sales or scaling performance. Works for Shopify stores and dropshipping businesses."
          />
        )}
        <form onSubmit={submit} className="mt-12 glass rounded-3xl p-8 sm:p-10 space-y-5 reveal">
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor="dg-name" className="text-xs text-muted-foreground">Your name</label>
              <input id="dg-name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required maxLength={80} className="mt-1 w-full bg-background/30 border border-border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
            <div>
              <label htmlFor="dg-url" className="text-xs text-muted-foreground">Store URL</label>
              <input id="dg-url" type="url" placeholder="https://yourstore.com" value={form.url} onChange={(e) => setForm({ ...form, url: e.target.value })} required maxLength={200} className="mt-1 w-full bg-background/30 border border-border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
          </div>
          <div>
            <label htmlFor="dg-issue" className="text-xs text-muted-foreground">Main issue</label>
            <select id="dg-issue" value={form.issue} onChange={(e) => setForm({ ...form, issue: e.target.value })} className="mt-1 w-full bg-background/30 border border-border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6">
        <div className="relative glass rounded-3xl p-10 sm:p-14 text-center overflow-hidden reveal">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            <Link to="/contact" className="inline-flex items-center gap-2 rounded-xl gradient-primary text-white px-7 py-3.5 text-sm font-semibold shadow-glow hover:opacity-95 transition">
...
/* ---------- BLOG ---------- */
function BlogCard({ p, className = "" }: { p: (typeof BLOG_POSTS)[number]; className?: string }) {
  return (
    <Link
      to="/blog/$slug"
      params={{ slug: p.slug }}
      className={`glass rounded-2xl overflow-hidden hover:shadow-glow transition group flex flex-col ${className}`}
    >
      <div className="relative aspect-[16/10] overflow-hidden bg-royal/20">
        ...
        <span className="absolute top-3 left-3 px-2.5 py-1 rounded-md text-[11px] font-medium bg-royal/60 backdrop-blur text-cyan border border-cyan/20">{p.category}</span>
      </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          ...
          <div className="flex gap-2">
            <button onClick={() => scroll(-1)} aria-label="Previous article" className="h-10 w-10 grid place-items-center rounded-xl glass hover:bg-foreground/10">
              <ChevronLeft className="h-4 w-4" />
            </button>
            <button onClick={() => scroll(1)} aria-label="Next article" className="h-10 w-10 grid place-items-center rounded-xl glass hover:bg-foreground/10">
              <ChevronRight className="h-4 w-4" />
            </button>
          </div>
        </div>
        <div ref={ref} className="mt-10 flex gap-5 overflow-x-auto snap-x snap-mandatory pb-6 -mx-4 px-4 scroll-smooth reveal" style={{ scrollbarWidth: "thin" }}>
          {BLOG_POSTS.map((p) => (
            <div key={p.slug} className="snap-start shrink-0 w-[280px] sm:w-[340px] lg:w-[380px] flex">
              <BlogCard p={p} className="w-full" />
            </div>
          ))}
        </div>
        <div className="mt-4 flex justify-center">
          <Link to="/blog" className="inline-flex items-center gap-2 rounded-xl glass hover:bg-foreground/10 px-6 py-3 text-sm font-semibold transition">
...
export function FAQ({ heading = true }: { heading?: boolean }) {
  ...
  return (
    <section id="faq" className={heading ? "py-20 sm:py-28" : "pb-20 pt-8"}>
      <div className="mx-auto max-w-3xl px-4 sm:px-6">
        {heading && <SectionHeading eyebrow="FAQ" title={<>Common questions</>} />}
...
export function Contact({ heading = true }: { heading?: boolean }) {
  ...
  return (
    <section id="contact" className={heading ? "py-20 sm:py-28" : "pb-20 pt-8"}>
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        {heading && <SectionHeading eyebrow="Contact" title={<>Ready to Fix Your Store? <span className="gradient-text">Let's Talk</span></>} />}
        <div className="mt-12 grid md:grid-cols-2 gap-6 reveal">
          <div className="glass rounded-3xl p-8 space-y-4">
            <a href={EMAIL} className="flex items-center gap-4 p-4 rounded-xl bg-background/30 border border-border hover:bg-foreground/5 transition">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">{EMAIL_ADDRESS}</div>
              </div>
            </a>
          ...
          <form onSubmit={submit} className="glass rounded-3xl p-8 space-y-4">
            <div>
              <label htmlFor="ct-name" className="text-xs text-muted-foreground">Your name</label>
              <input id="ct-name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required maxLength={80} className="mt-1 w-full bg-background/30 border border-border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
            <div>
              <label htmlFor="ct-email" className="text-xs text-muted-foreground">Email</label>
